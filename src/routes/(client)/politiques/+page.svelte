<script lang="ts">
	...
	$: ({ shop, policies, customizations } = $page.data);
	...
	$: hasAnyPolicy = policies?.terms_and_conditions || policies?.return_policy || policies?.delivery_policy || policies?.payment_terms;
</script>

<svelte:head>
	<title>Politiques de ventes - {shop.name}</title>
	<meta
		name="description"
		content="Politiques de ventes de {shop.name}. Consultez nos conditions générales de vente, politique de retour, livraison et paiement."
	/>
	<meta
		name="keywords"
		content="politiques de ventes, conditions générales, {shop.name}, pâtisserie, retour, livraison, paiement"
	/>
	<meta property="og:title" content="Politiques de ventes - {shop.name}" />
	<meta
		property="og:description"
		content="Politiques de ventes de {shop.name}"
	/>
...
>
	...
	<!-- Main content - Design style marketing -->
	<div class="px-4 pb-6 sm:pb-8 md:px-6 lg:px-8">
		<div class="mx-auto max-w-4xl">
			{#if hasAnyPolicy}
				<div class="space-y-8">
					<!-- Conditions générales de vente -->
					{#if policies.terms_and_conditions}
						<div class="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm sm:rounded-2xl sm:p-8">
							...
							<p
								class="whitespace-pre-wrap text-sm leading-[175%] text-neutral-600 sm:text-base"
			...
			{:else}
				<div class="py-12 text-center sm:py-16">
					...
					>
						Aucune politique de ventes n'est disponible pour le moment.
					</p>
				</div>
			{/if}
		</div>
	...
	<!-- Footer -->
	<ClientFooter {customizations} shopSlug={undefined} />
</div>
